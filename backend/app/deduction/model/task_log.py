...
class TaskLog(Base):
    ...
    id: Mapped[snowflake_id_key] = mapped_column(comment="日志记录唯一ID")

    task_id: Mapped[int] = mapped_column(comment="任务运行唯一ID")
    suffix: Mapped[int] = mapped_column(sa.Integer, comment="合成ID后缀")
    deduce_id: Mapped[int] = mapped_column(comment="推演方案ID")
